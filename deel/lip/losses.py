...
class MulticlassHinge(Loss):
    def __init__(
        ...
        Args:
            min_margin: positive float, margin to enforce.
            soft_hinge_tau: temperature applied in softmax for soft_hinge (default: set to 0 for classical hinge)
            reduction: passed to tf.keras.Loss constructor
            name: passed to tf.keras.Loss constructor
    ...
    @tf.function
    def call(self, y_true, y_pred):
        # binary GT to support weighted y_true values generated by process_labels_for_multi_gpu
        ...
        # Compute centered hinge (m/2-vYtrue)+(m/2+y_pred) for each output neuron and each sample
        ...
        if not self.use_soft_hinge:
            ...
            hinge = (1 - y_gt) * hinge
            # Divide by number of neurons to emulate reduce_mean with the following reduce_sum
