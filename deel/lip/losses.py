...
def multiclass_soft_hinge(y_true, y_pred, min_margin, soft_hinge_tau):
    """Compute the multi-class soft hinge margin loss. The hinge terms for predictions
    of logits other than the true class are weighted with their softmax values.
    ...
    Args:
        ...
        y_pred: tensor of predicted targets of shape (batch_size, # classes)
        min_margin: positive float, margin to enforce.
        soft_hinge_tau: positive float, temperature applied in softmax for soft_hinge
    ...
    pos = tf.reduce_sum(y_pred * mask, axis=-1, keepdims=True)
    neg = y_pred
    # Compute centered hinge (m/2-vYtrue)+(m/2+y_pred)
    # for each output neuron and each sample
    hinge = tf.nn.relu(min_margin / 2.0 - pos) + tf.nn.relu(min_margin / 2.0 + neg)
    # Compute soft hinge weights
    F_soft = tf.nn.softmax(
        tf.where(mask > 0, tf.float32.min, y_pred * soft_hinge_tau),
        axis=-1,
    )
    hinge *= F_soft
...
class MulticlassHinge(Loss):
    def __init__(
        ...
        if self.soft_hinge_tau > 0:
            self.fct_hinge = partial(
                multiclass_soft_hinge,
                min_margin=self.min_margin,
                soft_hinge_tau=self.soft_hinge_tau,
            )
        else:
            self.fct_hinge = partial(multiclass_hinge, min_margin=self.min_margin)

    @tf.function
    def call(self, y_true, y_pred):
        return self.fct_hinge(y_true, y_pred)
