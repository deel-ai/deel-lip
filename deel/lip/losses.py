...
def multiclass_hinge(y_true, y_pred, min_margin, soft_hinge_tau):
    ...
    pos = tf.reduce_sum(y_pred * y_gt, axis=1, keepdims=True)
    neg = y_pred
    ...
    if soft_hinge_tau > 0:
        ...
        F_soft = tf.nn.softmax(
            tf.where(y_gt > 0, -tf.float32.max, (y_pred) * soft_hinge_tau),
            axis=-1,
        )
        hinge = hinge * F_soft
    else:
        ...
        if (len(tf.shape(y_pred)) == 2) and (tf.shape(y_pred)[-1] != 1):
            factor = tf.cast(tf.shape(y_pred)[-1], y_pred.dtype)
        else:
            factor = 1.0
...
class MulticlassHinge(Loss):
    ...
    @tf.function
    def call(self, y_true, y_pred):
        return multiclass_hinge(y_true, y_pred, self.min_margin, self.soft_hinge_tau)
